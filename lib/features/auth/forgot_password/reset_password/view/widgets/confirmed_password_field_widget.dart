...
class ConfirmedPasswordFieldWidget extends StatefulWidget {
  const ConfirmedPasswordFieldWidget({super.key});

  @override
  State<ConfirmedPasswordFieldWidget> createState() =>
      _ConfirmedPasswordFieldWidgetState();
}

class _ConfirmedPasswordFieldWidgetState
    extends State<ConfirmedPasswordFieldWidget> {
  ...
  void _focusNodeListener() {
    if (!_focusNode.hasFocus) {
      context.read<ResetPasswordCubit>().onConfirmPasswordUnfocused();
    }
  }
  ...
  Widget build(BuildContext context) {
    final passwordError = context.select(
      (ResetPasswordCubit cubit) => cubit.state.confirmPassword.errorMessage,
    );

    final showPassword = context.select(
      (ResetPasswordCubit cubit) => cubit.state.showConfirmPassword,
    );
    ...
    return AppTextField(
      ...
      suffixIcon: Tappable(
        onTap: context.read<ResetPasswordCubit>().onShowConfirmPasswordChanged,
        child: Icon(
          showPassword
